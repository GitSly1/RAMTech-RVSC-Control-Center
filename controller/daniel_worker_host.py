...
def _execute_sem_daniel(api_key: str, mission: dict[str, Any], run_id: str, started: str) -> dict[str, Any]:
    ...
    runner = EngineeringMissionRunner(
        request=_worker_request_from_mission(mission),
        repo_root=SEMANTIQ_REPO,
        validations=(
    ...
    evidence.extend(runner.evidence_after_change(SEM_DANIEL_ALLOWED))
...
def _worker_request_from_mission(mission: dict[str, Any]):
    ...
    return WorkerRequest(
        ...
        allowed_paths=tuple(mission.get("allowed_paths", ())),
        acceptance_criteria=tuple(mission.get("acceptance_criteria", ())),
    )
...
def execute_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    prompt = (
        ...
        "Return a concise execution acknowledgement for this bounded mission, including agent id, work package, project, branch, objective, and acceptance criteria.\n\n"
        + json.dumps(mission, indent=2)
    )
    response = _openai_call(api_key, prompt)
    ended = _utc_now()
    provider_response_id = str(response.get("id", ""))
    model = str(response.get("model", DEFAULT_MODEL))
    status = str(response.get("status", "unknown"))
    evidence = [
        f"run_id:{run_id}", f"agent_id:{mission['agent_id']}", f"wp_id:{mission.get('wp_id', '')}",
        f"project:{mission.get('project', '')}", f"branch:{mission.get('work_branch', '')}",
        f"started_at:{started}", f"ended_at:{ended}", "provider:openai", f"model:{model}",
        f"provider_response_id:{provider_response_id}", f"provider_status:{status}",
    ]
    return {"success": status == "completed", "summary": f"DEV-001 Daniel live invocation completed for {mission.get('wp_id', '')}", "evidence": evidence, "retryable": False}
...
class DanielHandler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:
        ...
        try:
            ...
            encoded = json.dumps({"success": False, "summary": str(exc), "evidence": ["worker_host:daniel"], "retryable": False}).encode("utf-8")
